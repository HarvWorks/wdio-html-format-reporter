...
class HtmlReporter extends events.EventEmitter {
  constructor (baseReporter, config, options = {}) {
    ...
    this.on('runner:logtime', function (data) {
      ...
      spec.logTimes[data.output.suiteNumber - 1][data.output.browserName] = data.output.logTime;
    })
  }
  ...
  htmlOutput() {
    ...
    Handlebars.registerHelper('drawTableRows', function(spec, options) {
      ...
      console.log("suiteTitles", suiteTitles)
      let result = ""
      spec.logTimes.forEach((logTimeRow, index) => {
        ...
        spec.logTimeHeader.forEach((browser, jndex) => {
          if (jndex === 0) {
            console.log("suiteTitles[index]", suiteTitles[index])
            result = `${result}<td>${index + 1}: ${suiteTitles[index]}</td>`
          } else {
            const timeTook = logTimeRow[browser] ? logTimeRow[browser] : '-'
            result = `${result}<td>${timeTook}</td>`;
          }
        })
        result = `${result}</tr>`;
      })
