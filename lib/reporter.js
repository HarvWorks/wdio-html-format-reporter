...
class HtmlReporter extends events.EventEmitter {
  constructor (baseReporter, config, options = {}) {
    ...
    this.on('end', function () {
      ...
      let data = file.data;
      data.push(this.baseReporter.stats);
  ...
  htmlOutput() {
    ...
    fs.outputJsonSync(file.jsonfilePath, file.data);
  ...
  mergeData( data = [] ) {
    ...
    data.forEach( stats => {
      ...
      if (results._duration < stats._duration) {
      ...
      _.values(stats.runners).forEach( runner => {
        _.values(runner.specs).forEach( spec => {
          _.values(spec.suites).forEach( suite => {
            ...
            results.customSuite.specs.currentSpecs.suites[uuid] = suite;
          })

          const currentLogTimes = results.customSuite.specs.currentSpecs.logTimes;
          results.customSuite.specs.currentSpecs.logTimes = currentLogTimes.concat(spec.logTimes);

          const currentLogTimeHeader = results.customSuite.specs.currentSpecs.logTimeHeader
          results.customSuite.specs.currentSpecs.logTimeHeader = this.mergeArrays(currentLogTimeHeader, spec.logTimes);
        })
      })
