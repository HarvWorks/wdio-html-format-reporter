...
class HtmlReporter extends events.EventEmitter {
  constructor (baseReporter, config, options = {}) {
    ...
    this.on('runner:screenshot', function (runner) {
      ...
      const suiteKey = Object.keys(spec.suites)[Object.keys(spec.suites).length-1]
      const currentTestKey = Object.keys(spec.suites[suiteKey].tests)[Object.keys(spec.suites[suiteKey].tests).length-1]
    ...
    this.on('suite:end', function (suite) {
      ...
      if (tempResults.failing > 0) {
        spec.suites[suiteKey].status = "failing";
      ...
      if (tempResults.passing > 0) {
        spec.suites[suiteKey].status = "passing";
  ...
  getSuiteKey(data) {
    const spec = this.getSpec(data);
    return Object.keys(spec.suites)[Object.keys(spec.suites).length-1]
  }

  getCurrentTestKey(data) {
    const spec = this.getSpec(data);
    const suiteKey = this.getSuiteKey(data);
